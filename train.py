...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="weights/stable_diffusion/model.safetensors",
        required=False,
    ...
    parser.add_argument(
        "--save_sample_prompt",
        type=str,
        default="a picture represent lofi-music",
...
def main(args):
    ...
    text_encoder_ = stable_diffusion.text_encoder
    vae = stable_diffusion.vae
    unet = stable_diffusion.unet
    vae.requires_grad_(False)
    text_encoder_.requires_grad_(False)
    ...
    noise_scheduler = DDPMScheduler.from_config(stable_diffusion.scheduler.config)
    ...
    text_encoder_.to(accelerator.device, dtype=weight_dtype)
        ...
        for batch in tqdm(train_dataloader, desc="Caching latents"):
            with torch.no_grad():
                ...
                if args.train_text_encoder:
                    ...
                    text_encoder_cache.append(text_encoder_(batch["input_ids"])[0])
    ...
    def save_weights(step):
        ...
        if accelerator.is_main_process:
            pipeline_ = StableDiffusionPipeline.from_single_file(
                args.pretrained_model_name_or_path,
            ...
            pipeline_.scheduler = DDIMScheduler.from_config(pipeline_.scheduler.config)
            if is_xformers_available():
                pipeline_.enable_xformers_memory_efficient_attention()
            save_dir = os.path.join(args.output_dir, f"{step}")
            pipeline_.save_pretrained(save_dir)
            with open(os.path.join(save_dir, "args.json"), "w") as f:
                json.dump(args.__dict__, f, indent=2)

            if args.save_sample_prompt is not None:
                pipeline_ = pipeline_.to(accelerator.device)
                g_cuda = torch.Generator(device=accelerator.device).manual_seed(args.seed)
                pipeline_.set_progress_bar_config(disable=True)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                with text_enc_context:
                    if not args.not_cache_latents:
                        if args.train_text_encoder:
                            encoder_hidden_states = text_encoder_(batch[0][1])[0]
                        else:
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(unet.parameters(), text_encoder_.parameters())
                        if args.train_text_encoder
